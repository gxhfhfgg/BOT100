...
async def create_server_task(interaction, ram, cpu):
    await interaction.response.send_message(embed=discord.Embed(
        description=f"🚀 **Creating VPS Instance with {ram} RAM & {cpu} CPU Cores...**",
        color=0x00ff00
    ))

    user_id = str(interaction.user.id)
    admin_ids = {"1085944828883369984", "1119657947434332211"}  # Admins

    # Role-based resource limits
    member_limits = {"ram": ["4g"], "cpu": [2]}
    admin_limits = {
        "ram": ["4g", "5g", "6g", "8g", "10g", "12g", "16g", "32g", "64g"],
        "cpu": [4, 5, 6, 8, 10, 12, 16, 32, 64]
    }

    is_admin = user_id in admin_ids
    allowed_ram = admin_limits["ram"] if is_admin else member_limits["ram"]
    allowed_cpu = admin_limits["cpu"] if is_admin else member_limits["cpu"]

    if ram not in allowed_ram or cpu not in allowed_cpu:
        await interaction.followup.send(embed=discord.Embed(
            description="❌ **Unauthorized Resource Allocation!**\n"
                        f"🔹 **Members:** Max **4GB RAM, 2 Cores**\n"
                        f"🔹 **Admins:** Up to **64GB RAM, 64 Cores**",
            color=0xff0000
        ))
        return

    # Deploy VPS with requested RAM & CPU
    try:
        container_id = subprocess.check_output([
            "docker", "run", "-itd", "--privileged",
            "--memory", ram, "--cpus", str(cpu),
            "--hostname", "freevps", "--cap-add=ALL", "ubuntu-22.04-with-tmate"
        ]).strip().decode('utf-8')
        ...
        if ssh_session_line:
            ...
            await interaction.user.send(embed=discord.Embed(
                description=f"✅ **VPS Deployed Successfully!**\n"
...
@bot.tree.command(name="deploy", description="Deploy a VPS with custom RAM & CPU")
@app_commands.describe(ram="Amount of RAM (e.g., 4g, 8g, 16g)", cpu="Number of CPU cores (e.g., 2, 4, 8)")
async def deploy(interaction: discord.Interaction, ram: str, cpu: int):
    await create_server_task(interaction, ram, cpu)
