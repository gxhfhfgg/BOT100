...
async def adminnode(interaction: discord.Interaction):
    ...
    # Pagination (3 pages)
    pages = [[], [], []]
    for i, line in enumerate(vps_list):
        user, container_name, ssh_command = line.strip().split("|")
        entry = f"👤 **User ID:** `{user}`\n💾 **RAM:** Unknown\n🖥️ **CPU:** Unknown\n📜 **Container:** `{container_name}`\n🔑 **SSH:** `{ssh_command}`\n"
        pages[i % 3].append(entry)

    page_embeds = []
    for i, page in enumerate(pages):
        embed = discord.Embed(
            title=f"📜 VPS Instance List (Page {i + 1}/3)",
            description="\n\n".join(page) if page else "No data on this page.",
            color=0x00ff00
        )
        embed.set_footer(text="🚀 Use /delvps to remove a VPS.")
        page_embeds.append(embed)

    await interaction.response.send_message(embed=page_embeds[0])
    for embed in page_embeds[1:]:
        await interaction.followup.send(embed=embed)
