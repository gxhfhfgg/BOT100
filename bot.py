...
whitelist_ids = {"1119657947434332211"}  # Replace with actual admin IDs

@bot.tree.command(name="deploy", description="Creates a new VPS with RAM and CPU options.")
@app_commands.describe(ram="Amount of RAM (1-100GB for admins, 4GB for normal users)", 
                       cores="Number of CPU cores (1-100 for admins, 2 for normal users)")
async def deploy_ubuntu(interaction: discord.Interaction, ram: int = 4, cores: int = 2):
    await interaction.response.defer()

    userid = str(interaction.user.id)
    is_admin = userid in whitelist_ids  # Check if user is an admin
